...
import {
  createInvoiceSchema, createRefundSchema, listSchema, recordPaymentSchema,
  refundDecisionSchema, updateInvoiceSchema,
...
router.get('/payments', authorize('payment:view'), asyncHandler(async (req, res) => {
  res.json(await accountsService.listPayments(listSchema.parse(req.query)));
}));
router.post('/payments', authorize('payment:create'), validateBody(recordPaymentSchema), asyncHandler(async (req, res) => {
  const p = await accountsService.recordPayment(req.body);
...
router.patch('/payments/:id/confirm', authorize('payment:edit'), asyncHandler(async (req, res) => {
  const p = await accountsService.confirmPayment(req.params.id);
